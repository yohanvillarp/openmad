...
window.App.setNavbarOpen = function (open) {
...
window.App.updateNavbarActive = function () {
    const navbar = getNavbarEl();
    if (!navbar) return;
    ...
    if (window.matchMedia(NAVBAR_MQ).matches) {
        window.App.setNavbarOpen(false);
    }
};
...
window.App.renderNavbar = function () {
    ...
    <header class="navbar">
      ...
      <div class="navbar__panel" id="navbar-panel">
        ...
        <nav class="navbar__nav" aria-label="Secciones">
          <a href="#rutas" class="navbar__link">
            <span class="icon-ruta" aria-hidden="true"></span>
            <span>Rutas</span>
          </a>
          <a href="#conceptos" class="navbar__link">
          ...
          <a href="#perfil" class="navbar__link navbar__link--cta">
  ...
  `;
        ...
        if (typeof window.App.renderSearch === 'function') {
            window.App.renderSearch('navbar-search');
        }
    ...
    if (typeof window.App.renderProgress === 'function') {
...
function bindNavbarEvents() {
    ...
    toggle.addEventListener('click', () => {
        window.App.setNavbarOpen(!navbar.classList.contains('is-open'));
    });
    ...
    window.addEventListener('hashchange', window.App.updateNavbarActive);
}

document.addEventListener('DOMContentLoaded', () => {
    if (typeof window.App.renderNavbar === 'function') {
        window.App.renderNavbar();
    }
});
