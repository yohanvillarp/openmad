...
.DEFAULT_GOAL := help
.PHONY: help install dev-backend dev-web dev-vanilla build build-backend \
        build-web build-vanilla lint test test-e2e check audit \
...
help:
	...
	@echo    make build           Compila todo el monorepo (backend + web + vanilla)
	...
	@echo    make lint            Ejecuta linter en backend y apps/web
...
build: build-vanilla build-web build-backend
...
clean:
	...
	node -e "['backend/dist','apps/vanilla/dist','apps/web/dist'].forEach(d => require('fs').rmSync(d, {recursive: true, force: true}))"
