...
window.App.renderSearch = function (containerId) {
  ...
  `;
    ...
    const searchIndex = window.App.buildSearchIndex();
    let results = [];
    let activeIndex = -1;
    ...
    function openResult(item) {
        if (!item || window.App.isSearchItemLocked(item)) return;
        closeResults();
        input.value = '';
        window.location.hash = item.href;
        if (typeof window.App.setNavbarOpen === 'function') {
            window.App.setNavbarOpen(false);
        }
    }

    function renderResults() {
        const query = window.App.normalizeSearchText(input.value);
        activeIndex = -1;
        ...
        results = searchIndex
            .map((item) => ({ item: item, score: window.App.rankSearchItem(item, query) }))
        ...
        resultsBox.innerHTML = results.length
            ? results.map((item, index) => {
                const locked = window.App.isSearchItemLocked(item);
    ...
    function openFirstResult() {
        const firstAvailable = results.find((item) => !window.App.isSearchItemLocked(item));
