 ...
 * @returns {string}
 */
window.App.renderStepCard = function (step, options) {
    const item = step || {};
    const opts = options || {};
    ...
    const taskProgress = window.App.getStepTaskProgress(
        opts.routeId || '',
        opts.moduleId || '',
        item.id || ''
    );
    ...
    if (taskProgress.total) bits.push(`${taskProgress.done}/${taskProgress.total} mini tareas`);
    bits.push(window.App.formatXp(window.App.getStepXp()));
